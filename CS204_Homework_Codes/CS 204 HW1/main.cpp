...
/*
 ...
 #include <sstream>
 using namespace std;
 void printmatrix(const vector<vector<char>>& matrix) //Prints the matrix.
 {
     for (int i = 0; i < matrix.size(); ++i)
     {
         for (int j = 0; j < matrix[i].size(); ++j)
 ...
 void totalmovementcheck(const vector<vector<char>>& matrix, int rowsize, int colsize, bool& iteration, int& numofdash) //Checks for total movement available to be done in the matrix.
 {
     ...
     // Update iteration flag based on available movements
     iteration = (totalnumber > 0) && (numofdash != rowsize * colsize);
 }
 ...
 int main()
 {
     ...
     while (iteration) //this is the main movement function where all the magic happens.
     {
         ...
         char matrixletter = matrix[row][column];
         if (matrixletter == 'u' && upcheck(matrix, row) == true) //if its u checks if there is a viable movement and does it if there is.
         {
             int temprow = row - 1;
             bool turntime = true;
             while (temprow > -1 && turntime == true)
             {
                 if (matrix[temprow][column] == '-')
                 {
                     --temprow;
         ...
         else if (matrixletter == 'd' && downcheck(matrix, row, rowsize)) //its the same with the previous one just a different letter and way.
         {
             int temprow = row + 1;
             bool turntime = true;
             while (temprow < rowsize && turntime == true)
         ...
         else if (matrixletter == 'l' && leftcheck(matrix, column)) //same as before
         {
             int tempcol = column - 1;
             bool turntime = true;
             while (tempcol > -1 && turntime == true)
